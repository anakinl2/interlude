...
public class MercTicketManager
{
	...
	private void load()
	{
		...
		try
		{
			...
			while(rset.next())
			{
				...
				y = rset.getInt("y");
				z = rset.getInt("z");
				Residence siegeUnit = SiegeManager.getSiegeUnitByCoord(x, y);
	...
	public void removeTicket(L2ItemInstance item)
	{
		...
		Residence castle = CastleManager.getInstance().getCastleByIndex(getTicketCastleId(itemId));
