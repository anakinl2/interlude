...
public class SpawnTaskManager
{
	...
	{
		ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(new SpawnScheduler(), 2000, 2000);
	}
		...
		public void run()
		{
			if(_spawnTasksSize > 0)
				try
				{
					...
					{
						work.getSpawn().decreaseScheduledCount();
						if(work.getSpawn().isDoRespawn())
							work.getSpawn().respawnNpc(work);
					}
				}
				catch(Exception e)
				{
	...
	public String toString()
	{
		...
		{
			sb.append("Class/Name: ").append(container.getClass().getSimpleName()).append('/').append(container.actor.get().getName());
	...
	{
		private WeakReference<L2NpcInstance> actor;
		public long endtime;

		SpawnTask(L2NpcInstance cha, long delay)
		{
			actor = new WeakReference<L2NpcInstance>(cha);
			endtime = delay;
		}
