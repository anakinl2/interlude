...
public class SkillsEngine
{
	...
	private void hashFiles(String dirname, List<File> hash)
	{
		...
		if(!dir.exists())
		{
			_log.config(" ~ ERROR: Dir " + dir.getAbsolutePath() + " not exists");
			return;
		}
	...
	public List<L2Skill> loadSkills(File file)
	{
		if(file == null)
		{
			_log.config(" ~ ERROR: File not found!");
			return null;
		}
	...
	public L2Skill[][] loadAllSkills(int _maxid, int _maxLevel)
	{
		...
		for(File file : _skillFiles)
		{
			...
			for(L2Skill skill : s)
			{
				...
				if(result[skillIdIdx][skillLvlIdx] != null)
					new KeyAlreadyExistsException(" ~ ERROR: Unable to store skill " + skill).printStackTrace();
				else
		...
		_log.info(" ~ Loaded: " + total + " skill templates from XML files.");
		_log.info(" ~ Loaded: Max id=" + result.length + ", max level=" + maxLevel);
		_log.info("[ Skills Engine ]\n");
