...
public abstract class L2Object
{
	...
	public boolean isActionBlocked(String action)
	{
		...
		{
			for(L2Territory terr : getTerritories())
				if(terr.getZone() != null && terr.getZone().isActive() && terr.getZone().getType() == ZoneType.unblock_actions && terr.getZone().isActionBlocked(action))
					return false;
			for(L2Territory terr : getTerritories())
				if(terr.getZone() != null && terr.getZone().isActive() && terr.getZone().getType() != ZoneType.unblock_actions && terr.getZone().isActionBlocked(action))
					return true;
	...
	{
		return isInZoneIncludeZ(ZoneType.water) && !isInZone(ZoneType.no_water) && !isInBoat();
	}

	public boolean isInZoneIncludeZ(ZoneType type)
	{
		//FastList<L2Territory> territories = L2World.getActiveTerritory(getX(), getY());
		if(_territories != null)
			for(L2Territory terr : getTerritories())
	...
	public int getWaterZ()
	{
		if(!isPlayer() || isInBoat() || _territories == null)
			return -1;
		for(L2Territory terr : getTerritories())
			if(terr != null && terr.getZone() != null && terr.getZone().getType() == ZoneType.no_water && terr.isInside(getX(), getY()))
				return -1;
		int z = GeoEngine.getHeight(getLoc());
		for(L2Territory terr : getTerritories())
			if(terr != null && terr.getZone() != null && terr.getZone().getType() == ZoneType.water && terr.isInside(getX(), getY(), z))
				return terr.getZmax();
		return -1;
	}
