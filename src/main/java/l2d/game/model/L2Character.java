...
import static l2d.game.ai.CtrlEvent.EVT_FORGET_OBJECT;
...
import l2d.game.model.L2Skill.TriggerActionType;
...
import l2d.game.serverpackets.Attack;
import l2d.game.serverpackets.AutoAttackStart;
import l2d.game.serverpackets.AutoAttackStop;
import l2d.game.serverpackets.ChangeMoveType;
import l2d.game.serverpackets.ChangeWaitType;
import l2d.game.serverpackets.CharMoveToLocation;
import l2d.game.serverpackets.FlyToLocation;
import l2d.game.serverpackets.FlyToLocation.FlyType;
import l2d.game.serverpackets.L2GameServerPacket;
import l2d.game.serverpackets.MagicSkillCanceled;
...
import l2d.game.serverpackets.StopMove;
import l2d.game.serverpackets.SystemMessage;
import l2d.game.serverpackets.TeleportToLocation;
import l2d.game.serverpackets.ValidateLocation;
import l2d.game.serverpackets.VehicleDeparture;
...
import l2d.game.taskmanager.RegenTaskManager;
...
public abstract class L2Character extends L2Object
{
	...
	public void callSkill(final L2Skill skill, final FastList<L2Character> targets, final boolean useActionSkills)
	{
		try
		{
			...
			{
				final ConcurrentLinkedQueue<L2Skill> SkillsOnMagicAttack = _skillsOnAction.get(TriggerActionType.OFFENSIVE_MAGICAL_SKILL_USE);
				if(skill.isOffensive() && skill.isMagic() && SkillsOnMagicAttack != null)
					for(final L2Skill sk : SkillsOnMagicAttack)
						if(Rnd.chance(sk.getChanceForAction(TriggerActionType.OFFENSIVE_MAGICAL_SKILL_USE)))
						{
				...
				if(skill.isOffensive())
					for(final L2Character target : targets)
						...
						{
							final ConcurrentLinkedQueue<L2Skill> SkillsOnUnderSkillAttack = target.getTriggerableSkills().get(TriggerActionType.UNDER_SKILL_ATTACK);
							if(SkillsOnUnderSkillAttack != null)
								for(final L2Skill sk : SkillsOnUnderSkillAttack)
									if(sk != skill && Rnd.chance(sk.getChanceForAction(TriggerActionType.UNDER_SKILL_ATTACK)))
									{
				...
				final ConcurrentLinkedQueue<L2Skill> SkillsOnMagicSupport = _skillsOnAction.get(TriggerActionType.SUPPORT_MAGICAL_SKILL_USE);
				if(!skill.isOffensive() && skill.isMagic() && SkillsOnMagicSupport != null)
					for(final L2Skill sk : SkillsOnMagicSupport)
						if(Rnd.chance(sk.getChanceForAction(TriggerActionType.SUPPORT_MAGICAL_SKILL_USE)))
	...
	public void doCast(final L2Skill skill, L2Character target, final boolean forceUse)
	{
		...
		final boolean forceBuff = skill.getSkillType() == SkillType.FORCE_BUFF;

		if(forceBuff)
		{
			final L2Effect spell_force = target.getEffectList().getEffectByType(EffectType.SpellForce);
			final L2Effect battle_force = target.getEffectList().getEffectByType(EffectType.BattleForce);
		...
		if(mpConsume1 > 0)
			if(skillType == SkillType.MUSIC)
			{
				final double inc = mpConsume1 / 2;
				double add = 0;
				for(final L2Effect e : getEffectList().getAllEffects())
					if(e.getSkill().getId() != skill.getId() && (e.getSkill().getSkillType() == SkillType.MUSIC) && e.getTimeLeft() > 30000)
						add += inc;
				mpConsume1 += add;
	...
	{
		if(skill.oneTarget() && !(target == actor || skill.getTargetType() == SkillTargetType.TARGET_SELF) || skill.getTargetType() == SkillTargetType.TARGET_AREA || skill.getTargetType() == SkillTargetType.TARGET_MULTIFACE)
		{
	...
	public int getAbnormalEffect()
	{
		...
		if(getEffectList().getEffectByType(EffectType.SilentMove) != null)
	...
	public final FastTable<L2Skill> getSkillsByType(final SkillType type)
	{
	...
	private void onHitTimer(final L2Character target, final int damage, final boolean crit, final boolean miss, final boolean soulshot, final boolean shld, final boolean unchargeSS)
	{
		...
		if(!isAttackAborted())
		{
			...
			if(!miss && damage > 0)
			{
				...
				if(!target.isDead())
				{
					if(_skillsOnAction != null)
						if(crit)
						{
							final ConcurrentLinkedQueue<L2Skill> SkillsOnCrit = _skillsOnAction.get(TriggerActionType.CRIT);
							if(SkillsOnCrit != null)
								for(final L2Skill skill : SkillsOnCrit)
									if(crit && Rnd.chance(skill.getChanceForAction(TriggerActionType.CRIT)))
									{
						...
						{
							final ConcurrentLinkedQueue<L2Skill> SkillsOnAttack = _skillsOnAction.get(TriggerActionType.ATTACK);
							if(SkillsOnAttack != null)
								for(final L2Skill skill : SkillsOnAttack)
									if(Rnd.chance(skill.getChanceForAction(TriggerActionType.ATTACK)))
									{
										final L2Character aimingTarget = skill.getAimingTarget(this, target);
										final FastList<L2Character> targets = skill.getTargets(this, aimingTarget, false);
										callSkill(skill, targets, false);
					...
					if(target.getTriggerableSkills() != null)
					{
						final ConcurrentLinkedQueue<L2Skill> SkillsOnUnderAttack = target.getTriggerableSkills().get(TriggerActionType.UNDER_ATTACK);
						if(SkillsOnUnderAttack != null)
							for(final L2Skill skill : SkillsOnUnderAttack)
								if(Rnd.chance(skill.getChanceForAction(TriggerActionType.UNDER_ATTACK)))
								{
			...
			{
				final ConcurrentLinkedQueue<L2Skill> SkillsOnUnderEvasion = target.getTriggerableSkills().get(TriggerActionType.UNDER_MISSED_ATTACK);
				if(SkillsOnUnderEvasion != null)
					for(final L2Skill skill : SkillsOnUnderEvasion)
						if(Rnd.chance(skill.getChanceForAction(TriggerActionType.UNDER_MISSED_ATTACK)))
						{
							final L2Character aimingTarget = skill.getAimingTarget(target, this);
							final FastList<L2Character> targets = skill.getTargets(target, aimingTarget, false);
							target.callSkill(skill, targets, false);
	...
	public void reduceCurrentHp(double i, final L2Character attacker, final L2Skill skill, final boolean awake, final boolean standUp, final boolean directHp, final boolean canReflect)
	{
		...
		if(getEffectList().getEffectByType(EffectType.Petrification) != null || getEffectList().getEffectByType(EffectType.Invulnerable) != null)
		{
		...
		if(standUp && isPlayer())
		{
			...
			if(isFakeDeath())
			{
				final L2Effect fakeDeath = getEffectList().getEffectByType(EffectType.FakeDeath);
				if(fakeDeath == null)
					stopFakeDeath();
	...
	{
		for(final TriggerActionType e : newSkill.getTriggerActions().keySet())
		{
			if(_skillsOnAction == null)
