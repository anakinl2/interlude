...
import l2d.game.model.entity.SevenSignsFestival.SevenSignsFestival.FestivalStatus;
...
public class FestivalManager implements Runnable
{
	...
	private FestivalStatus _status;
	private long _elapsed;

	public FestivalManager(FestivalStatus status)
	{
		_status = status;
	}

	public FestivalManager(FestivalStatus status, long elapsed)
	{
		_status = status;
		_elapsed = elapsed;
	}
	...
	public synchronized void run()
	{
		...
		switch(_status)
		{
			case Begining:
				...
				ThreadPoolManager.getInstance().scheduleGeneral(new FestivalManager(FestivalStatus.Started), SevenSignsFestival.FESTIVAL_SIGNUP_TIME);
				break;
			case Started:
				...
				ThreadPoolManager.getInstance().scheduleGeneral(new FestivalManager(FestivalStatus.FirstSpawn), SevenSignsFestival.FESTIVAL_FIRST_SPAWN);
				break;
			case FirstSpawn:
				...
				ThreadPoolManager.getInstance().scheduleGeneral(new FestivalManager(FestivalStatus.FirstSwarm, _elapsed), SevenSignsFestival.FESTIVAL_FIRST_SWARM - SevenSignsFestival.FESTIVAL_FIRST_SPAWN);
				break;
			case FirstSwarm:
				...
				ThreadPoolManager.getInstance().scheduleGeneral(new FestivalManager(FestivalStatus.SecondSpawn, _elapsed), SevenSignsFestival.FESTIVAL_SECOND_SPAWN - SevenSignsFestival.FESTIVAL_FIRST_SWARM);
				break;
			case SecondSpawn:
				...
				ThreadPoolManager.getInstance().scheduleGeneral(new FestivalManager(FestivalStatus.SecondSwarm, _elapsed), SevenSignsFestival.FESTIVAL_SECOND_SWARM - SevenSignsFestival.FESTIVAL_SECOND_SPAWN);
				break;
			case SecondSwarm:
				for(L2DarknessFestival festivalInst : SevenSignsFestival.getFestivalInstances().values())
					festivalInst.moveMonstersToCenter();
				...
				ThreadPoolManager.getInstance().scheduleGeneral(new FestivalManager(FestivalStatus.ChestSpawn, _elapsed), SevenSignsFestival.FESTIVAL_CHEST_SPAWN - SevenSignsFestival.FESTIVAL_SECOND_SWARM);
				break;
			case ChestSpawn:
				...
				ThreadPoolManager.getInstance().scheduleGeneral(new FestivalManager(FestivalStatus.Ending, _elapsed), SevenSignsFestival.FESTIVAL_LENGTH - _elapsed);
				break;
			case Ending:
