...
public abstract class Residence
{
	...
	private List<ResidenceFunction> _functions = new FastList<ResidenceFunction>();
	...
	public void spawnDoor(boolean isDoorWeak)
	{
		for(int i = 0; i < getDoors().size(); i++)
		{
			...
			if(door.isOpen()) // Открытые двери закрываем
				door.closeMe();
		}
	...
	FastMap<Integer, Integer> doorUpgrades = new FastMap<Integer, Integer>();
	...
	{
		if(activeChar.getClanId() != getOwnerId() && !activeChar.isGM())
			return;
	...
	/**
	 * Выдвет клану-владельцу скилы резиденции
	 */
	...
	private void preLoadFunctions()
	{
		try
		{
			File file = new File(Config.DATAPACK_ROOT + "/data/residence.xml");
					...
					for(Node d1 = n1.getFirstChild(); d1 != null; d1 = d1.getNextSibling())
						if("residence".equalsIgnoreCase(d1.getNodeName()) && Integer.parseInt(d1.getAttributes().getNamedItem("id").getNodeValue()) == getId())
							for(Node s1 = d1.getFirstChild(); s1 != null; s1 = s1.getNextSibling())
							{
								...
								int lease = (int) (getNodeValue(s1, "lease") * Config.RESIDENCE_LEASE_MULTIPLIER);
	...
	private void loadFunctions()
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				function.setEndTimeInMillis(rs.getInt("endTime") * 1000);
	...
	private void StartAutoTaskForFunction(ResidenceFunction function)
	{
		if(getOwnerId() != 0)
			try
			{
				...
				else if(adena != null && function.isInDebt() && adena.getCount() >= function.getLease()) // if player didn't pay before add extra fee
				{
			...
			{
				System.out.println("StartAutoTaskForFunction: id: " + getId() + ", type: " + function.getType());
				e.printStackTrace();
			}
	}
