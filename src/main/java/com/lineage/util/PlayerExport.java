...
public abstract class PlayerExport
{
	...
	public static String export(String name)
	{
		...
		{
			if(s.equals("clanid") || s.equals("clan_privs") || s.equals("deletetime") || s.equals("title"))
				e.append(" " + s + "='0'");
		...
		Integer nid = 0;
		ArrayList<HashMap<String, Object>> set;
		e.append("	<items>\n");
	...
	{
		e.append("	<" + query[0] + ">\n");
		ArrayList<HashMap<String, Object>> set = mysql.getAll("SELECT * FROM " + query[0] + " WHERE " + query[1] + "=" + query[2]);
		for(HashMap<String, Object> map : set)
		{
			e.append("		<row");
			for(String s : map.keySet())
