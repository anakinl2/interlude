...
public class Geodata
{
	...
	{
		common.SetDummyUseMem(512 * 0x100000);
		common.init();
		...
		bench_canSee(iterations);
		freeMemAfter = Runtime.getRuntime().freeMemory() + Runtime.getRuntime().maxMemory() - Runtime.getRuntime().totalMemory();
		common.log.info("Used Mem " + (freeMemBefore - freeMemAfter) / 1048576 + " Mb");

		System.gc();
		freeMemBefore = Runtime.getRuntime().freeMemory() + Runtime.getRuntime().maxMemory() - Runtime.getRuntime().totalMemory();
		bench_canSeeOld(iterations);
		freeMemAfter = Runtime.getRuntime().freeMemory() + Runtime.getRuntime().maxMemory() - Runtime.getRuntime().totalMemory();
		common.log.info("Used Mem " + (freeMemBefore - freeMemAfter) / 1048576 + " Mb");
	...
	public static long bench_canSee(final int iterations)
	{
		common.log.info("Benchmarking Geodata::canSee | loading geo map: " + geo_map_x + "_" + geo_map_y + "...");
		long result = System.currentTimeMillis();

		final short[] curr = new short[GeoEngine.MAX_LAYERS + 1], next = new short[GeoEngine.MAX_LAYERS + 1];
		int k = 0;
		...
		for(int x = xblock_first; x < xblock_first + 1920; x += 4)
		{
			...
			for(int y = yblock_first; y < yblock_first + 1920; y += 4)
			{
				...
				for(int i = 1; i <= curr[0]; i++)
				{
					...
					{
						GeoEngine.canSee(x, y, ((curr[i] & 0x0fff0) >> 1), x + 128, y + 128, ((next[i] & 0x0fff0) >> 1), false);
						k++;
					}
				}
			}
	...
	public static long bench_canSeeOld(final int iterations)
	{
		...
		for(int x = xblock_first; x < xblock_first + 1920; x += 4)
		{
			...
			for(int y = yblock_first; y < yblock_first + 1920; y += 4)
			{
				...
				for(int i = 1; i <= curr[0]; i++)
				{
					...
					for(int j = 1; j <= next[0]; j++)
					{
						GeoEngine.canSeeCoord(x, y, ((curr[i] & 0x0fff0) >> 1), x + 128, y + 64, ((next[i] & 0x0fff0) >> 1), false);
						k++;
					}
				}
			}
