...
public class Formulas
{
	...
	private static class FuncPCriticalDmgRcpt extends Func
	{
		...
		public void calc(final Env env)
		{
			...
			if(i != null && i.getItemType() == L2Armor.ArmorType.HEAVY && (env.character.getSkillLevel(L2Skill.SKILL_HEAVY_ARMOR_MASTERY1) > 0 || env.character.getSkillLevel(L2Skill.SKILL_HEAVY_ARMOR_MASTERY2) > 0 || env.character.getSkillLevel(L2Skill.SKILL_HEAVY_ARMOR_MASTERY3) > 0 || env.character.getSkillLevel(L2Skill.SKILL_HEAVY_ARMOR_MASTERY4) > 0))
				env.value -= 20;
		}
	}
	...
	private static class FuncFatalBlowMul extends Func
	{
		...
		public void calc(final Env env)
		{
			...
			if(env.target.isPlayer())
			{
				...
				if(pcTarget.isWearingArmor(L2Armor.ArmorType.HEAVY))
				...
				else if(pcTarget.isWearingArmor(L2Armor.ArmorType.MAGIC))
				...
				else if(pcTarget.isWearingArmor(L2Armor.ArmorType.LIGHT))
	...
	private static class FuncSDefPlayers extends Func
	{
		...
		public void calc(final Env env)
		{
			...
			if(shld == null || shld.getItemType() != L2Weapon.WeaponType.NONE)
				return;
