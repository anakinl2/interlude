...
import com.lineage.game.model.instances.L2SepulcherMonsterInstance;
import com.lineage.game.model.instances.L2SepulcherNpcInstance;
...
public class FourSepulchersManager
{
	...
	private static final String QUEST_ID = "_620_FourGoblets";
	...
	private static HashMap<Integer, L2Zone> _zone = new HashMap<Integer, L2Zone>();

	protected byte _newCycleMin = 55;

	protected boolean _firstTimeRun;

	private static FastList<L2Player> _playersInside = new FastList<L2Player>();
	...
	{
		_instance = this;
		_log.info("Initializing Four Sepulcher manager");
		_instance.init();
	...
	{
		for(int i = 706001; i <= 706024; i++)
			_zone.put(i, ZoneManager.getInstance().getZoneById(L2Zone.ZoneType.dummy, i, false));
	}
	...
	protected void cleanUp()
	{
		...
		for(L2Player player : _playersInside)
			player.teleToLocation(MapRegion.getTeleToClosestTown(player));
	...
	protected void spawnManagers()
	{
		...
		for(L2Spawn spawnDat; i <= 31924; i++)
		{
			if(i < 31921 || i > 31924)
				continue;
			L2NpcTemplate template1 = NpcTable.getTemplate(i);
			if(template1 == null)
				continue;
			try
			{
				...
				_log.info("FourSepulchersManager: Spawned " + spawnDat.getLastSpawn().getName());
			}
	...
	private void loadMysteriousBox()
	{
		...
		try
		{
			...
			FiltredPreparedStatement statement = con.prepareStatement("SELECT `id`, `count`, `npc_templateid`, `locx`, `locy`, `locz`, `heading`, `respawn_delay`, `key_npc_id` FROM `four_sepulchers_spawnlist` WHERE `spawntype` = ? ORDER BY `id`");
	...
	private void initKeyBoxSpawns()
	{
		...
		for(int keyNpcId : _keyBoxNpc.keySet())
			try
			{
				template = NpcTable.getTemplate(_keyBoxNpc.get(keyNpcId));
				if(template != null)
				{
	...
	private void loadPhysicalMonsters()
	{
		...
		try
		{
			...
			FiltredPreparedStatement statement1 = con.prepareStatement("SELECT DISTINCT `key_npc_id` FROM `four_sepulchers_spawnlist` WHERE `spawntype` = 1 ORDER BY `key_npc_id`");
			...
			while(rset1.next())
			{
				...
				while(rset2.next())
				{
					template1 = NpcTable.getTemplate(rset2.getInt("npc_templateid"));
					if(template1 != null)
					{
						spawnDat = new L2Spawn(template1);
	...
	private void loadDukeMonsters()
	{
		...
		try
		{
			...
			FiltredPreparedStatement statement1 = con.prepareStatement("SELECT DISTINCT `key_npc_id` FROM `four_sepulchers_spawnlist` WHERE `spawntype` = 5 ORDER BY `key_npc_id`");
			ResultSet rset1 = statement1.executeQuery();
			while(rset1.next())
			{
				int keyNpcId = rset1.getInt("key_npc_id");

				FiltredPreparedStatement statement2 = con.prepareStatement("SELECT `id`, `count`, `npc_templateid`, `locx`, `locy`, `locz`, `heading`, `respawn_delay`, `key_npc_id` FROM `four_sepulchers_spawnlist` WHERE `key_npc_id` = ? AND `spawntype` = 5 ORDER BY `id`");
				statement2.setInt(1, keyNpcId);
				ResultSet rset2 = statement2.executeQuery();
	...
	private void loadEmperorsGraveMonsters()
	{
		...
		try
		{
			...
			FiltredPreparedStatement statement1 = con.prepareStatement("SELECT DISTINCT `key_npc_id` FROM `four_sepulchers_spawnlist` WHERE `spawntype` = 6 ORDER BY `key_npc_id`");
			ResultSet rset1 = statement1.executeQuery();
			while(rset1.next())
			{
				int keyNpcId = rset1.getInt("key_npc_id");

				FiltredPreparedStatement statement2 = con.prepareStatement("SELECT `id`, `count`, `npc_templateid`, `locx`, `locy`, `locz`, `heading`, `respawn_delay`, `key_npc_id` FROM `four_sepulchers_spawnlist` WHERE `key_npc_id` = ? AND `spawntype` = 6 ORDER BY `id`");
				statement2.setInt(1, keyNpcId);
				ResultSet rset2 = statement2.executeQuery();
	...
	protected void initLocationShadowSpawns()
	{
		...
		for(int i = 0; i <= 3; i++)
		{
			template = NpcTable.getTemplate(_shadowSpawnLoc[locNo][i][0]);
			if(template != null)
				try
				{
					spawnDat = new L2Spawn(template);
				...
				{
					_log.warning("Error on InitLocationShadowSpawns");
					e.printStackTrace();
				}
			else
	...
	protected void initExecutionerSpawns()
	{
		...
		for(int keyNpcId : _victim.keySet())
			try
			{
				template = NpcTable.getTemplate(_victim.get(keyNpcId));
				if(template != null)
				{
					spawnDat = new L2Spawn(template);
					spawnDat.setAmount(1);
	...
	public synchronized void tryEntry(L2NpcInstance npc, L2Player player)
	{
		...
		if(Config.FS_PARTY_MEMBER_COUNT > 1)
		{
			...
			for(L2Player mem : player.getParty().getPartyMembers())
			{
				...
				if(!mem.isQuestContinuationPossible())
					return;
			}
		}
	...
	private void entry(int npcId, L2Player player)
	{
		...
		if(Config.FS_PARTY_MEMBER_COUNT > 1)
		{
			...
			for(L2Player mem : members)
			{
				...
				mem.getInventory().destroyItemByItemId(ENTRANCE_PASS, 1, true);
				if(mem.getInventory().getItemByItemId(ANTIQUE_BROOCH) == null)
					mem.getInventory().addItem(USED_PASS, 1, mem.getObjectId(), "Quest");

				L2ItemInstance hallsKey = mem.getInventory().getItemByItemId(CHAPEL_KEY);
				if(hallsKey != null)
					mem.getInventory().destroyItemByItemId(CHAPEL_KEY, hallsKey.getCount(), true);
			}
		...
		else
		{
			...
			player.getInventory().destroyItemByItemId(ENTRANCE_PASS, 1, true);
			if(player.getInventory().getItemByItemId(ANTIQUE_BROOCH) == null)
				player.getInventory().addItem(USED_PASS, 1, player.getObjectId(), "Quest");

			L2ItemInstance hallsKey = player.getInventory().getItemByItemId(CHAPEL_KEY);
			if(hallsKey != null)
				player.getInventory().destroyItemByItemId(CHAPEL_KEY, hallsKey.getCount(), true);
		...
		if(monsterList != null)
		{
			...
			for(L2Spawn spawnDat : monsterList)
			{
				...
				if(spawnKeyBoxMob)
				{
					try
					{
						L2NpcTemplate template = NpcTable.getTemplate(18149);
						if(template != null)
						{
				...
				{
					mob = (L2SepulcherMonsterInstance) spawnDat.doSpawn(false);
					spawnDat.stopRespawn();
				}
	...
	public void spawnArchonOfHalisha(int npcId)
	{
		...
		if(monsterList != null)
		{
			for(L2Spawn spawnDat : monsterList)
			{
				L2SepulcherMonsterInstance mob = (L2SepulcherMonsterInstance) spawnDat.doSpawn(false);
				spawnDat.stopRespawn();
	...
	public void checkAnnihilated(final L2Player player)
	{
		...
		for(L2Player _player : player.getParty().getPartyMembers())
			if(!_player.isDead())
				return;
	...
	public void deleteAllMobs()
	{
		...
		for(L2NpcInstance mob : _allMobs)
			...
			{
				_log.warning("FourSepulchersManager: Failed deleting mob.");
				e.printStackTrace();
			}
		_allMobs.clear();
	}
	...
	protected void closeAllDoors()
	{
		for(int doorId : _hallGateKeepers.values())
			...
			{
				_log.warning("FourSepulchersManager: Failed closing door");
				e.printStackTrace();
			}
	}
	...
	protected class ChangeEntryTime implements Runnable
	{
		public void run()
		{
			...
			ThreadPoolManager.getInstance().executeGeneral(new ManagerSay());
	...
	public void showHtmlFile(L2Player player, String file, L2NpcInstance npc, L2Player member)
	{
