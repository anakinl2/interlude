...
public abstract class L2Playable extends L2Character
{
	...
	public boolean checkPvP(final L2Character target, final L2Skill skill)
	{
		...
		if(skill != null)
		{
			if(skill.altUse())
				return false;
			if(skill.getSkillType().equals(L2Skill.SkillType.BEAST_FEED))
				return false;
			if(skill.getTargetType() == L2Skill.SkillTargetType.TARGET_UNLOCKABLE)
				return false;
			if(skill.getTargetType() == L2Skill.SkillTargetType.TARGET_CHEST)
				return false;
		}
		...
		if(isInZone(L2Zone.ZoneType.peace_zone) || target.isInZone(L2Zone.ZoneType.peace_zone) || isInZoneBattle() || target.isInZoneBattle())
			return false;
		if(isInZone(L2Zone.ZoneType.Siege) && target.isInZone(L2Zone.ZoneType.Siege))
			return false;
	...
	public boolean isAutoAttackable(final L2Character attacker)
	{
		...
		if(pcAttacker != null)
		{
			...
			if(isInZone(L2Zone.ZoneType.Siege) && attacker.isInZone(L2Zone.ZoneType.Siege))
			{
				if(clan1 == null || clan2 == null)
					return true;
	...
	public void callSkill(final L2Skill skill, final FastList<L2Character> targets, final boolean useActionSkills)
	{
		...
		if(useActionSkills && !skill.altUse() && !skill.getSkillType().equals(L2Skill.SkillType.BEAST_FEED))
