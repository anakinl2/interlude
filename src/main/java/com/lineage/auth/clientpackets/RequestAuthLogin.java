	...
	public void runImpl()
	{
		...
		else if(status.state == LoginController.State.IN_USE)
		{
			...
			if(lc.isAccountInLoginServer(_user))
				lc.removeAuthedLoginClient(_user).close(LoginFail.LoginFailReason.REASON_ACCOUNT_IN_USE);
