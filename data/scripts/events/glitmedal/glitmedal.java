...
public class glitmedal extends Functions implements ScriptFile
{
	...
	{
		return Helper.IsActive("glitter");
	}
	...
	public void startEvent()
	{
		...
		if(Helper.SetActive("glitter", true))
		{
			spawnEventManagers();
			System.out.println("Event 'L2 Medal Collection Event' started.");
			Announcements.getInstance().announceByCustomMessage("scripts.events.glitmedal.glitmedal.AnnounceEventStarted", null);
		}
		else
	...
	public void stopEvent()
	{
		...
		if(!player.getPlayerAccess().IsEventGm)
			return;
		if(Helper.SetActive("glitter", false))
		{
			unSpawnEventManagers();
			System.out.println("Event 'L2 Medal Collection Event' stopped.");
			Announcements.getInstance().announceByCustomMessage("scripts.events.glitmedal.glitmedal.AnnounceEventStoped", null);
		}
		else
	...
	{
		if(_active)
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.glitmedal.glitmedal.AnnounceEventStarted", null);
	}
	...
	private void spawnEventManagers()
	{
		...
		Helper.SpawnNPCs(EVENT_MANAGER_ID1, EVENT_MANAGERS1, _spawns);
		Helper.SpawnNPCs(EVENT_MANAGER_ID2, EVENT_MANAGERS2, _spawns);
	}
	...
	public static void OnDie(L2Character cha, L2Character killer)
	{
		...
		{
			int count = Util.rollDrop(1, 1, Config.EVENT_GLITTMEDAL_NORMAL_CHANCE * killer.getPlayer().getRateItems() * ((L2MonsterInstance) cha).getTemplate().rateHp * 10000, true);
			if(count > 0)
