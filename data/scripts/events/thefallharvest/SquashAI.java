...
public class SquashAI extends Fighter
{
	...
	{
		new L2DropData(1539, 1, 5, 15000, 1), // Greater Healing Potion
	...
	public SquashAI(L2Character actor)
	{
		...
		Functions.npcShoutCustomMessage(getActor(), textOnSpawn[Rnd.get(textOnSpawn.length)], null);
	...
	protected void onEvtSeeSpell(L2Skill skill, L2Character caster)
	{
		...
		switch(_tryCount)
		{
			case 0:
				...
				{
					_nectar++;
					Functions.npcShout(actor, textSuccess0[Rnd.get(textSuccess0.length)]);
					actor.broadcastPacket(new MagicSkillUse(actor, actor, Squash_Level_up, 1, NECTAR_REUSE, 0));
				}
				else
				{
					Functions.npcShout(actor, textFail0[Rnd.get(textFail0.length)]);
			...
			case 1:
				...
				{
					Functions.npcShout(actor, textTooFast[Rnd.get(textTooFast.length)]);
					return;
				}
				_tryCount++;
				_lastNectarUse = System.currentTimeMillis();
				...
				{
					_nectar++;
					Functions.npcShout(actor, textSuccess1[Rnd.get(textSuccess1.length)]);
					actor.broadcastPacket(new MagicSkillUse(actor, actor, Squash_Level_up, 1, NECTAR_REUSE, 0));
				}
				else
				{
					Functions.npcShout(actor, textFail1[Rnd.get(textFail1.length)]);
					actor.broadcastPacket(new MagicSkillUse(actor, actor, Squash_Poisoned, 1, NECTAR_REUSE, 0));
				}
				break;
			case 2:
				...
				{
					_nectar++;
					Functions.npcShout(actor, textSuccess2[Rnd.get(textSuccess2.length)]);
					actor.broadcastPacket(new MagicSkillUse(actor, actor, Squash_Level_up, 1, NECTAR_REUSE, 0));
				}
				else
				{
					Functions.npcShout(actor, textFail2[Rnd.get(textFail2.length)]);
					actor.broadcastPacket(new MagicSkillUse(actor, actor, Squash_Poisoned, 1, NECTAR_REUSE, 0));
				}
				break;
			case 3:
				...
				{
					_nectar++;
					Functions.npcShout(actor, textSuccess3[Rnd.get(textSuccess3.length)]);
					actor.broadcastPacket(new MagicSkillUse(actor, actor, Squash_Level_up, 1, NECTAR_REUSE, 0));
				}
				else
				{
					Functions.npcShout(actor, textFail3[Rnd.get(textFail3.length)]);
					actor.broadcastPacket(new MagicSkillUse(actor, actor, Squash_Poisoned, 1, NECTAR_REUSE, 0));
				}
				break;
			case 4:
				...
				{
					_nectar++;
					Functions.npcShout(actor, textSuccess4[Rnd.get(textSuccess4.length)]);
					actor.broadcastPacket(new MagicSkillUse(actor, actor, Squash_Level_up, 1, NECTAR_REUSE, 0));
				}
				else
				{
					Functions.npcShout(actor, textFail4[Rnd.get(textFail4.length)]);
					actor.broadcastPacket(new MagicSkillUse(actor, actor, Squash_Poisoned, 1, NECTAR_REUSE, 0));
				}
				...
				_polimorphTask = ThreadPoolManager.getInstance().scheduleAi(new PolimorphTask(), NECTAR_REUSE, false);
				break;
		}
	}
	...
	protected void onEvtAttacked(L2Character attacker, int damage)
	{
		...
		if(actor != null && Rnd.chance(5))
			Functions.npcShout(actor, textOnAttack[Rnd.get(textOnAttack.length)]);
	}
	...
	protected void onEvtDead()
	{
		...
		switch(_npcId)
		{
			case Low_Quality_Squash:
				dropMod *= 1;
				Functions.npcShout(actor, "The pampkin opens!!!");
				Functions.npcShout(actor, "ya yo! Opens! Good thing many...");
				break;
			...
			default:
				dropMod *= 0;
				Functions.npcShout(actor, "Ouch, if I had died like this, you could obtain nothing!");
				Functions.npcShout(actor, "The news about my death shouldn't spread, oh!");
				break;
		}
		...
		if(dropMod > 0)
		{
			...
			{
				ArrayList<ItemToDrop> itd = d.roll(null, dropMod, false);
