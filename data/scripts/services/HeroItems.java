...
public class HeroItems extends Functions implements ScriptFile
{
	...
	public static void getweapon(String[] var)
	{
		...
		if(player.isHero())
		{
			L2ItemInstance old_item_inv = null;
			L2ItemInstance old_item_wh = null;
			for(String heroItem[] : HERO_ITEMS)
			{
				int itemId = Integer.parseInt(heroItem[0]);
				old_item_inv = player.getInventory().getItemByItemId(itemId);
				old_item_wh = player.getWarehouse().findItemId(itemId);
				if(old_item_inv != null && Config.SERVICES_HERO_WEAPONS_EXCHANGE_ENABLED)
				{
					removeItem(player, itemId, 1);
					additem(item);
					break;
				}
				else if(old_item_wh != null && Config.SERVICES_HERO_WEAPONS_EXCHANGE_ENABLED)
				{
					player.getWarehouse().destroyItem(itemId, 1);
					additem(item);
					break;
				}
				else if ((old_item_inv != null || old_item_wh != null) && !Config.SERVICES_HERO_WEAPONS_EXCHANGE_ENABLED)
				{
					String lang = player.getVar("lang@");
					if(lang.equalsIgnoreCase("en"))
						player.sendMessage("You already have weapons in this month, it could change next month when you become a hero again");
					else
						player.sendMessage("Вы уже выбрали оружие в этом месяце, сменить его можно в следующем месяце если вы станете героем повторно");
					break;
				}
			}
			if(old_item_inv == null && old_item_wh == null)
				additem(item);
		}
	}
	...
	public static void rendershop(String[] val)
	{
		...
		else if(Integer.parseInt(val[0]) >= 0 && Integer.parseInt(val[0]) <= HERO_ITEMS.length)
		{
			...
			htmltext += "<CENTER><button value=Back action=\"bypass -h scripts_services.HeroItems:rendershop list\" width=40 height=15 back=sek.cbui92 fore=sek.cbui92></CENTER>";
	...
	public static String itemz()
	{
		...
		if(player.getPlayer().isHero())
		{
			...
			if(player.getInventory().getItemByItemId(6842) == null && player.getWarehouse().findItemId(6842) == null)
				if(lang.equalsIgnoreCase("en"))
					append += "<br>[scripts_services.HeroItems:getcir|Get Hair Accessory.]<br>";
				else
					append += "<br>[scripts_services.HeroItems:getcir|Получить корону героя.]<br>";
			if(lang.equalsIgnoreCase("en"))
				append += "[scripts_services.HeroItems:rendershop list|Select a hero weapon.]";
			else
				append += "[scripts_services.HeroItems:rendershop list|Выбрать оружие героя.]";
			if(lang.equalsIgnoreCase("en"))
				append = "<br><font color=\"LEVEL\">Hero rewards:</font><br1>" + append;
			else
				append = "<br><font color=\"LEVEL\">Награда героя:</font><br1>" + append;
		}

		return append;
	}

	public static String DialogAppend_31690(Integer val)
	{
		if(val != 0)
			return "";
		return itemz();
	...
	public static String DialogAppend_31769(Integer val)
	{
		if(val != 0)
			return "";
		return itemz();
	...
	public static String DialogAppend_31770(Integer val)
	{
		if(val != 0)
			return "";
		return itemz();
	...
	public static String DialogAppend_31771(Integer val)
	{
		if(val != 0)
			return "";
		return itemz();
	...
	public static String DialogAppend_31772(Integer val)
	{
		if(val != 0)
			return "";
		return itemz();
	...
	public static String DialogAppend_31773(Integer val)
	{
		if(val != 0)
			return "";
		return itemz();
