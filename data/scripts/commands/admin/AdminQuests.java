...
public class AdminQuests implements IAdminCommandHandler, ScriptFile
{
	...
	public static void showqs(String[] var)
	{
		if(var.length < 2)
			return;
	...
	private static final PrintfFormat fmtSetButton = new PrintfFormat("<button value=\"Set\" action=\"bypass -h admin_quest %d %s %s %s %s\" width=30 height=20 back=\"sek.cbui94\" fore=\"L2UI_ct1.button_df\">");
	private static final PrintfFormat fmtFOOT = new PrintfFormat("<br><br><br><center><button value=\"Clear Quest\" action=\"bypass -h admin_quest %d CLEAR %s\" width=100 height=20 back=\"sek.cbui94\" fore=\"L2UI_ct1.button_df\"> <button value=\"Quests List\" action=\"bypass -h scripts_commands.admin.AdminQuests:showql %s\" width=100 height=20 back=\"sek.cbui94\" fore=\"L2UI_ct1.button_df\"></center>");
	...
	private static boolean ShowQuestState(QuestState qs, L2Player activeChar)
	{
		...
		replyMSG.append(fmtHEAD.sprintf(new Object[] { qs.getQuest().getDescr(), id }));
	...
	public static void showql(String[] var)
	{
		if(var.length < 1)
			return;
		L2Player activeChar = (L2Player) self;
	...
	private static final PrintfFormat fmtListNew = new PrintfFormat("<tr><td><edit var=\"new_quest\" width=100 height=12></td><td><button value=\"Add\" action=\"bypass -h admin_quest $new_quest STATE started %s\" width=40 height=20 back=\"sek.cbui94\" fore=\"L2UI_ct1.button_df\"></td></tr>");
	...
	private boolean cmd_Var(Quest _quest, String[] wordList, L2Player activeChar)
	{
		...
		if(qs == null)
		{
			...
			activeChar.sendMessage("//quest id|name STATE created|started|completed [target]");
			return false;
		}
	...
	private boolean cmd_State(Quest _quest, String[] wordList, L2Player activeChar)
	{
		...
		{
			activeChar.sendMessage("USAGE: //quest id|name STATE created|started|completed [target]");
			return false;
		}

		State state = _quest.getStateByID(wordList[3]);
		if(state == null)
		{
